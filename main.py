...
def pseudonimizar_texto(texto):
    ...
    ent_positions = [(ent.start_char, ent.end_char, ent.label_)
                     for ent in doc.ents]
    ...
    # Itera sobre as posições das entidades nomeadas, substituindo os nomes
    # por asteriscos
    for start, end, label in reversed(ent_positions):
        if label in [
                'PER', 'LOC']:  # Verifica se a entidade é uma pessoa ou um local
            # Substitui o texto pelo pseudônimo
...
# Texto de exemplo
texto_exemplo = (
    "O aplicante de nome José da Silva que mora na Rua das marrecas e tem número de telefone "
    "+55 (21) 9999-1234. Contato pelo e-mail jose@example.com."
)
...
# Saida = O aplicante de nome **** ** ***** que mora na Rua das ******** e
# tem número de telefone ************. Contato pelo e-mail
# *********@*****.***.
